...
locals {
  database = {
    ...
    # DATABASE_URL will be generated at runtime for IAM auth with actual tokens
    DATABASE_URL                              = var.database_passwordless_aws_use_iam ? null : (var.database_host != null ? "postgresql://${var.database_user}${var.database_password != null ? ":${var.database_password}" : ""}@${var.database_host}:5432/${var.database_name}${var.database_parameters != null ? "?${var.database_parameters}" : ""}" : null)
  }
