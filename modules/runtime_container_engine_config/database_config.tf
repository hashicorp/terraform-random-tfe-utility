...
locals {
  database = {
    ...
    # Add DATABASE_URL for AWS IAM authentication - use placeholder password for config validation
    DATABASE_URL                              = var.database_host != null ? "postgresql://${var.database_user}${var.database_password != null ? ":${var.database_password}" : ":aws-iam-auth"}@${var.database_host}:5432/${var.database_name}${var.database_parameters != null ? "?${var.database_parameters}" : ""}" : null
  }
