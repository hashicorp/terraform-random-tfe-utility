...
locals {
  ...
  env = merge(
    ...
    local.vault_configuration,
    {
      ...
      TFE_HOSTNAME_SECONDARY        = var.hostname_secondary
      ...
      TFE_TLS_CERT_FILE_SECONDARY   = var.cert_file_secondary
      TFE_TLS_CIPHERS               = var.tls_ciphers
      TFE_TLS_KEY_FILE              = var.key_file
      TFE_TLS_KEY_FILE_SECONDARY    = var.key_file_secondary
      ...
      TFE_OIDC_HOSTNAME_CHOICE      = var.oidc_hostname_choice
    }
  )
